...
def classify_image(img_data, file_path=None):
    imgs = get_cropped_image(file_path, img_data)
    result = []
    ...
    for img in imgs:
        ...
        if __model is not None:
            result.append({
                'class': class_num_to_name(__model.predict(final)[0]),
...
def load_saved_artefacts():
    ...
    with open("./artefacts/class_dictionary.json", "r") as f:
        __class_name_to_number = json.load(f)
        __class_number_to_name = {v:k for k,v in __class_name_to_number.items()}

    global __model
    if __model is None:
        with open('./artefacts/saved_model.pkl', 'rb') as f:
            __model = joblib.load(f)
    print("Loading saved artefacts..done")


def get_cropped_image(path, b64_data):
...
if __name__ == "__main__":
    ...
    print(classify_image(get_image(), None))